...
int pushToQueue(queue *q,node *n)
{
...
int pushToStack(stack *s,node *n)
{
    ...
    if (n == NULL)
        ...
        while (!NULL)
        {
            while (queue1->head != NULL) {
                ...
                if (tmp->left != NULL)
                    pushToQueue(queue2, tmp->left);
                else
                    pushToQueue(queue2, n);

                if (tmp->right != NULL)
                    pushToQueue(queue2, tmp->right);
                else
                    pushToQueue(queue2, n);
            }
...
//нерекурсивная реализация прямого обхода дерева
void preorderTraversal(tree *t) {
    ...
    while (s->top) {
        ...
        if (tmp->right) pushToStack(s, tmp->right);
        if (tmp->left) pushToStack(s, tmp->left);
    }
